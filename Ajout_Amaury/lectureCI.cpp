...
int run(int argc, char **argv)
{
  ...
  ros::Publisher imuPub = n.advertise<sensor_msgs::Imu>("/ouca?",1000);
  ...
  imuMsg.orientation_covariance[0] = 20.0;
  imuMsg.orientation_covariance[4] = 20.0;
  imuMsg.orientation_covariance[8] = 20.0;
  imuMsg.angular_velocity_covariance[0] = 25.0;
  imuMsg.angular_velocity_covariance[4] = 25.0;
  imuMsg.angular_velocity_covariance[8] = 25.0;
  imuMsg.linear_acceleration_covariance[0] = 20.0;
  imuMsg.linear_acceleration_covariance[4] = 20.0;
  imuMsg.linear_acceleration_covariance[8] = 20.0;
    ...
    serial_amaury::CI_msg msg;
		...
		if(resultVec.size()!=0){
			...
			if(!resultVec[0].compare("INFO") && resultVec.size()>1){
				...
				ROS_INFO("BBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBBB");
				imuMsg.header.stamp = ros::Time::now();
			}
			
			
			else if(!resultVec[0].compare("F")){
				ROS_INFO("FFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFFF");
				...
				if(!(msg.type).compare("A")){
					...
					msg.x = msg.x - correctionGyrX;
					msg.y = msg.y - correctionGyrY;
					msg.z = msg.z - correctionGyrZ;
